...
parent->child_load_success = load (file_name_, &if_.eip, &if_.esp);
...
process_exit (void)
{
  ...
  opened_file_list_destroy (current);
  page_destroy_mmap_list (&current->mmap_file_list);
  page_table_destroy (&current->pt);
  file_close (current->process_file);
...
static bool load_segment (struct file *file, off_t offs, uint8_t *upage,
...
{
  struct thread *current, *tmp_thread;
  ...
  if (file == NULL) 
  {
    tmp_thread = thread_get_thread_from_filename (file_name);
    ...
    file = tmp_thread->process_file;
  }

  current->process_file = file_reopen (file);
  file_deny_write (current->process_file);

  /* Read and verify executable header. */
  if (file_read (current->process_file, &ehdr, sizeof ehdr) != sizeof ehdr
  ...
  for (i = 0; i < ehdr.e_phnum; i++) 
  {
    ...
    if (file_ofs < 0 || file_ofs > file_length (current->process_file))
      goto done;
    file_seek (current->process_file, file_ofs);

    if (file_read (current->process_file, &phdr, sizeof phdr) != sizeof phdr)
      goto done;
    ...
    switch (phdr.p_type) 
    {
      ...
      case PT_SHLIB:
        goto done;
      case PT_LOAD:
        if (validate_segment (&phdr, current->process_file)) 
        {
          ...
          if (!load_segment (current->process_file, file_page, (void *) mem_page,
    ...
    uint32_t read_bytes, uint32_t zero_bytes, bool writable) 
{
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
  {
    ...
    if (!page_add_file_to_page_table (file, ofs, upage, page_read_bytes, 
...
setup_stack (void **esp, char *file_name, char *save_ptr) 
{
  ...
  *esp = PHYS_BASE;
  ...
  current->filename = (char *) malloc (buffer_size * sizeof (char));
  memcpy (current->filename, file_name, buffer_size);
  push_parse_arguments (esp, file_name, save_ptr);
